      ...
      describe "Enumerators" do
        context ".status_pending" do
          it "returns Pending" do
            _(model_klass.status_pending).must_equal("Pending")
          end
        end

        context "#status_pending" do
          it "returns Pending" do
            _(new_model.status_pending).must_equal("Pending")
          end
        end

        context ".status_options" do
          it "returns the expected Array" do
            _(model_klass.status_options).must_equal(status_options_array)
          end
        end

        context "#status_options" do
          it "returns the expected Array" do
            _(new_model.status_options).must_equal(status_options_array)
          end
        end

        context ".humanized_statuses_list" do
          it "returns the expected String" do
            _(model_klass.humanized_statuses_list).must_equal(
        ...
        context "#humanized_statuses_list" do
          it "returns the expected String" do
            _(new_model.humanized_statuses_list).must_equal(
              humanized_statuses_list)
          end
        end
      end
      ...
      describe "Setters" do
        context "#set_status_running" do
          it "sets the status to Running" do
            ...
            _(pending_model.status).must_equal("Running")
          end
        end
        ...
        context "#set_status_running!" do
          it "sets the status to Running and saves the record" do
            pending_model.set_status_running!
            _(pending_model.status).must_equal("Running")
            _(pending_model.reload.status).must_equal("Running")
          end
        end
      end
        ...
        context "#status?" do
          context "GIVEN a single status" do
            it "returns true, GIVEN a matching status" do
              _(pending_model.status?("Pending")).must_equal(true)
            end

            it "returns false, GIVEN a non-matching status" do
              _(pending_model.status?("Not Pending")).must_equal(false)
            end

            it "returns false, GIVEN nil" do
              _(pending_model.status?(nil)).must_equal(false)
            end
          end

          context "GIVEN an array of statuses" do
            it "returns true, GIVEN at least one matching status" do
              _(pending_model.status?(["Pending", "Not Pending"])).
                must_equal(true)
            end

            it "returns false, GIVEN no matching statuses" do
              _(pending_model.status?(["Not Pending", "Also Not Pending"])).
                must_equal(false)
            end

            it "returns false, GIVEN an empty Array" do
              _(pending_model.status?([])).must_equal(false)
            end
          end
        end
        ...
        context "#not_status?" do
          context "GIVEN a single status" do
            it "returns false, GIVEN a matching status" do
              _(pending_model.not_status?("Pending")).must_equal(false)
            end

            it "returns true, GIVEN a non-matching status" do
              _(pending_model.not_status?("Not Pending")).must_equal(true)
            end

            it "returns true, GIVEN nil" do
              _(pending_model.not_status?(nil)).must_equal(true)
            end
          end

          context "GIVEN an array of statuses" do
            it "returns false, GIVEN at least one matching status" do
              _(pending_model.not_status?(["Pending", "Not Pending"])).
                must_equal(false)
            end

            it "returns true, GIVEN no matching statuses" do
              _(pending_model.not_status?(["Not Pending", "Also Not Pending"])).
                must_equal(true)
            end

            it "returns true, GIVEN an empty Array" do
              _(pending_model.not_status?([])).must_equal(true)
            end
          end
        end
      ...
      describe "Validations" do
        context "GIVEN Presence is not required" do
          ...
          it "fails validation, GIVEN a blank status" do
            subject.status = nil
            subject.validate
            _(subject.errors[:status]).wont_include("can't be blank")
          end
    ...
    describe "CUSTOM OPTIONS" do
      ...
      describe "Enumerators" do
        context ".lifecycle_state_pending" do
          it "returns Pending" do
            _(model_klass.lifecycle_state_pending).must_equal("Pending")
          end
        end

        context "#lifecycle_state_pending" do
          it "returns Pending" do
            _(new_model.lifecycle_state_pending).must_equal("Pending")
          end
        end

        context ".lifecycle_state_options" do
          it "returns the expected Array" do
            _(model_klass.lifecycle_state_options).must_equal(
        ...
        context "#lifecycle_state_options" do
          it "returns the expected Array" do
            _(new_model.lifecycle_state_options).must_equal(
              lifecycle_state_options_array)
          end
        end
        ...
        context ".humanized_lifecycle_states_list" do
          it "returns the expected String" do
            _(model_klass.humanized_lifecycle_states_list).must_equal(
        ...
        context "#humanized_lifecycle_states_list" do
          it "returns the expected String" do
            _(new_model.humanized_lifecycle_states_list).must_equal(
              humanized_lifecycle_states_list)
          end
        end
      end
      ...
      describe "Setters" do
        context "#set_lifecycle_state_running" do
          it "sets the lifecycle_state to Running" do
            ...
            _(pending_model.lifecycle_state).must_equal("Running")
          end
        end
        ...
        context "#set_lifecycle_state_running!" do
          it "sets the lifecycle_state to Running and saves the record" do
            pending_model.set_lifecycle_state_running!
            _(pending_model.lifecycle_state).must_equal("Running")
            _(pending_model.reload.lifecycle_state).must_equal("Running")
          end
        end
      end
      ...
      describe "Predicates" do
        context "#lifecycle_state?" do
          it "returns true, GIVEN a matching lifecycle_state" do
            _(pending_model.lifecycle_state?("Pending")).must_equal(true)
          end

          it "returns false, GIVEN a non-matching lifecycle_state" do
            _(pending_model.lifecycle_state?("Not Pending")).must_equal(false)
          end
        end

        context "#not_lifecycle_state?" do
          it "returns false, GIVEN a matching lifecycle_state" do
            _(pending_model.not_lifecycle_state?("Pending")).must_equal(false)
          end

          it "returns true, GIVEN a non-matching lifecycle_state" do
            _(pending_model.not_lifecycle_state?("Not Pending")).must_equal(
              true)
          end
        end
        ...
        context "#lifecycle_state_pending?" do
          it "returns true, GIVEN a pending lifecycle_state" do
            _(pending_model.lifecycle_state_pending?).must_equal(true)
          end

          it "returns false, GIVEN a non-pending lifecycle_state" do
            subject = [running_model, completed_model].sample
            _(subject.lifecycle_state_pending?).must_equal(false)
          end
        end

        context "#not_lifecycle_state_pending?" do
          it "returns false, GIVEN a pending lifecycle_state" do
            _(pending_model.not_lifecycle_state_pending?).must_equal(false)
          end

          it "returns true, GIVEN a non-pending lifecycle_state" do
            subject = [running_model, completed_model].sample
            _(subject.not_lifecycle_state_pending?).must_equal(true)
          end
        end
      end
      ...
      describe "Validations" do
        context "GIVEN Presence is required" do
          ...
          it "fails validation, GIVEN a blank lifecycle_state" do
            ...
            subject.validate
            _(subject.errors[:lifecycle_state]).must_include("can't be blank")
          end

          it "passes validation, GIVEN a lifecycle_state" do
            subject.set_lifecycle_state_pending.validate
            _(subject.errors[:lifecycle_state]).wont_include("can't be blank")
          end
        end
        ...
        context "GIVEN Inclusion is not required" do
          ...
          it "passes validation, GIVEN an invalid lifecycle_state" do
            ...
            subject.validate
            _(subject.errors[:lifecycle_state]).wont_include(
              "must be one of Pending, Running, or Completed")
          end
